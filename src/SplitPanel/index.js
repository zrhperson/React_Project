...
class SplitPanel extends Component {
  ...
  render() {
    ...
    const direct = this.props.direct
    const lineClassName = direct == 'ver' ? 'bocomui-split-panel__verline' : 'bocomui-split-panel__horline'
    ...
    return (
      <div ref="container" className={classnames('bocomui-split-panel', className)} {...other}>
        <div ref="top" style={topStyle} className="bocomui-split-panel--top">{items[0]}</div>
        <div ref="line" className={lineClassName} onMouseDown={::this.handleMouseDown}></div>
        <div ref="bottom" style={bottomStyle} className="bocomui-split-panel--bottom">{items[1]}</div>
      </div>
    )
  }
