...
class MultipleSelect extends Component {
    ...
    return options.map((option, i) => {
      return (
        <li key={i} className={classnames({
          'bocomui-multiple-select__option': true,
          'bocomui-multiple-select__option--active': this.state.index === i
        })}>
          {option}
        </li>
      )
  ...
  render() {
    ...
    return (
      <SelectDropdown
        className={classnames('bocomui-multiple-select', className)}
        title={Title}
        url={url}
      ...
      >
        {
          options.length ? (
            <ul>
              <li className={classnames({
                'bocomui-multiple-select__option': true,
                'bocomui-multiple-select__option--active': index === -1
              })}>
                <Checkbox
                  checked={isAll}
                  block
          ...
          ) : <div className="bocomui-multiple-select__empty">{noOptionsContent}</div>
        }
      </SelectDropdown>
    )
  }
