...
class DataTable extends Component {
    ...
    const Theads = columns.map((item, i) => {
      ...
      return (
        <th
          key={i}
          width={item.width}
          className={classnames({'bocomui-datatable__th--sortable': sortable})}
          onClick={() => sortable && this.handleSort(item.key)}
        >
          {title}
          {sortable && <Icon className="bocomui-datatable__sort-symbol" type={icon} />}
        </th>
      )
    })
    return Theads
  ...
  render() {
    ...
    return (
      <Fetch
        className={classnames('bocomui-datatable', className)}
        defaultHeight={70}
        url={this.getUrl()}
        onSuccess={::this.handleLoad}
        {...other}
      >
        <table className="bocomui-table">
          <thead>
          ...
          <tbody>
            {
              ...
              currentPageData.map(::this.getRow) : (
                <tr>
                  <td colSpan={columns.length} className="bocomui-datatable__empty">
                    {noDataContent}
                  </td>
                </tr>
              )
